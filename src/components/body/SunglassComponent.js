...
const SunglassComponent = () => {
    ...
    const getSunglasses= async()=>{
        ...
        setSunglasses(result.data.data)
        setIsLoading(0)
    }

    const handleSearch = (event) => {
        setSunglasses(sunglasses.filter(product => 
    ...
    <div className='container'>
        ...
        <div className='row d-flex justify-content-center'>
            {sunglasses.map((product)=>(
                ...
                <button className='btn btn-secondary'><Link to={`/sunglass/${product._id}`} style={{textDecoration:"none", color:'whitesmoke'}} >See More</Link></button>
                </div>
        </div>
        ))}
        </div>
...
export default SunglassComponent