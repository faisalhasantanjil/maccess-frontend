...
const WatchComponet = () => {
    ...
    const getWatches= async()=>{
        ...
        const result = await axios.get('https://maccess001.onrender.com/watch');
        setWatches(result.data.data)
        setIsLoading(0)
    }

    const handleSearch = (event) => {
        setWatches(watches.filter(product => 
        ...
        <div className='row d-flex justify-content-center'>
            {
                watches.map((product)=>(
                    <div className= "card col-sm-12 col-md-6 col-lg-4 m-3 border-top-0 border-end-0" key={product._id} style={{boxShadow:"5px 5px #edebeb"}}>
                        ...
                        <button className='btn btn-secondary'><Link to={`/watch/${product._id}`} style={{textDecoration:"none", color:'whitesmoke'}} >See More</Link></button>
                        </div>
                    </div>
                ))
            }
